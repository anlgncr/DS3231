...
DS3231::DS3231(uint8_t device_address){
...
void DS3231::getTime(dateTime* my_dt){
	...
	Wire.requestFrom(my_device_address, (uint8_t)3); 
    if (Wire.available()){
...
void DS3231::getDate(dateTime* my_dt){
	...
	Wire.requestFrom(my_device_address, (uint8_t)4); 
    if (Wire.available()){
...
float DS3231::getTemperature(){
	...
	Wire.requestFrom(my_device_address, (uint8_t)2); 
    if (Wire.available()){
        upper = Wire.read();
		lower = Wire.read();
	}
